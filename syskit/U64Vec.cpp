...
//!
U64Vec::U64Vec(const U64Vec& vec):
Growable(vec)
{
...
//!
U64Vec::U64Vec(const U64Vec& vec, size_t startAt, size_t itemCount):
Growable(vec)
{
    item_ = new item_t[capacity()];
...
//!
U64Vec::U64Vec(unsigned int capacity, int growBy):
Growable(capacity, growBy)
{

    // Allocate all items. Initialize each item when used.
    item_ = new item_t[U64Vec::capacity()];
    numItems_ = 0;
}
...
//! be dropped.
//!
const U64Vec& U64Vec::operator =(const U64Vec& vec)
{
...
//! further.
//!
U64Vec::item_t* U64Vec::detachRaw()
{
    item_t* raw = item_;
    item_ = 0;
    numItems_ = 0;
...
//!
bool U64Vec::addAtIndex(size_t index, item_t item, bool maintainOrder)
{
...
//!
bool U64Vec::find(item_t item, size_t& foundIndex) const
{
    bool found = false;
...
//!
bool U64Vec::resize(unsigned int newCap)
{
    bool ok;
    if (numItems_ > newCap)
    {
...
//!
bool U64Vec::rm(item_t item, bool maintainOrder)
{
    size_t foundIndex;
...
//!
bool U64Vec::rmFromIndex(size_t index, item_t& removedItem, bool maintainOrder)
{
...
//!
bool U64Vec::search(item_t item, size_t& foundIndex) const
{
