...
//!
U16Vec::U16Vec(const U16Vec& vec):
Growable(vec)
{
...
//!
U16Vec::U16Vec(const U16Vec& vec, size_t startAt, size_t itemCount):
Growable(vec)
{
    item_ = new item_t[capacity()];
...
//!
U16Vec::U16Vec(unsigned int capacity, int growBy):
Growable(capacity, growBy)
{

    // Allocate all items. Initialize each item when used.
    item_ = new item_t[U16Vec::capacity()];
    numItems_ = 0;
}
...
//! be dropped.
//!
const U16Vec& U16Vec::operator =(const U16Vec& vec)
{
...
//! further.
//!
U16Vec::item_t* U16Vec::detachRaw()
{
    item_t* raw = item_;
    item_ = 0;
    numItems_ = 0;
...
//!
bool U16Vec::addAtIndex(size_t index, item_t item, bool maintainOrder)
{
...
//!
bool U16Vec::find(item_t item, size_t& foundIndex) const
{
    bool found = false;
...
//!
bool U16Vec::resize(unsigned int newCap)
{
    bool ok;
    if (numItems_ > newCap)
    {
...
//!
bool U16Vec::rm(item_t item, bool maintainOrder)
{
    size_t foundIndex;
...
//!
bool U16Vec::rmFromIndex(size_t index, item_t& removedItem, bool maintainOrder)
{
...
//!
bool U16Vec::search(item_t item, size_t& foundIndex) const
{
