 ...
 */
#ifndef SYSKIT_U64_VEC_HPP
#define SYSKIT_U64_VEC_HPP
...
//! vector of 64-bit numbers
class U64Vec: public Growable
    //!
    //! A class representing an unsorted vector of 64-bit numbers. The vector
...
public:
    ...
    // Constructors.
    U64Vec(const U64Vec& vec);
    U64Vec(const U64Vec& vec, size_t startAt, size_t itemCount);
    U64Vec(unsigned int capacity = DefaultCap, int growBy = 0);

    // Operators.
    const U64Vec& operator =(const U64Vec& vec);
    bool operator !=(const U64Vec& vec) const;
    bool operator ==(const U64Vec& vec) const;
    ...
    unsigned int add(const U64Vec& vec);
    unsigned int add(const U64Vec& vec, size_t startAt, size_t itemCount);
    ...
    unsigned int reset(const U64Vec& vec, size_t startAt, size_t itemCount);
    void reset();
    ...
    void sort(U64Vec& sorted, bool reverseOrder = false) const;
...
inline U64Vec::item_t U64Vec::operator [](size_t index) const
{
    return item_[index];
}
...
//! Peek at given index and return the residing item. Don't do any error
//! checking. Behavior is unpredictable if given index is invalid.
inline U64Vec::item_t& U64Vec::operator [](size_t index)
{
    return item_[index];
}

//! Return true if this vector does not equal given vector.
inline bool U64Vec::operator !=(const U64Vec& vec) const
{
...
//! Peek at given index and return the residing item. Don't do any error
//! checking. Behavior is unpredictable if given index is invalid.
inline U64Vec::item_t U64Vec::peek(size_t index) const
{
    return item_[index];
}
...
//! further.
inline U64Vec::item_t* U64Vec::detachRaw(unsigned int& numItems)
{
    numItems = numItems_;
...
inline bool U64Vec::addIfNotFound(item_t item)
{
    size_t foundIndex;
...
inline bool U64Vec::find(item_t item) const
{
    size_t foundIndex;
...
inline bool U64Vec::rmFromIndex(size_t index, bool maintainOrder)
{
    item_t removedItem;
...
inline bool U64Vec::rmTail(item_t& removedItem)
{
...
inline bool U64Vec::search(item_t item) const
{
    size_t foundIndex;
...
inline bool U64Vec::truncate(size_t numItems)
{
...
inline const U64Vec::item_t* U64Vec::raw() const
{
    return item_;
}
...
inline unsigned int U64Vec::add(const U64Vec& vec)
{
...
inline unsigned int U64Vec::add(const U64Vec& vec, size_t startAt, size_t itemCount)
{
...
inline unsigned int U64Vec::findIndex(item_t item) const
{
    size_t foundIndex;
...
inline unsigned int U64Vec::numItems() const
{
    return numItems_;
}
...
//! grow.
inline unsigned int U64Vec::reset(const U64Vec& vec, size_t startAt, size_t itemCount)
{
    numItems_ = 0;
    unsigned int numAdds = add(vec.item_ + startAt, itemCount);
    return numAdds;
...
inline void U64Vec::setItem(size_t index, item_t item)
{
    item_[index] = item;
}
...
inline void U64Vec::setItem(size_t index, item_t item, item_t& replacedItem)
{
...
inline void U64Vec::sort(bool reverseOrder)
{
    U64Heap::sort(item_, numItems_, reverseOrder);
}
...
inline void U64Vec::sort(U64Vec& sorted, bool reverseOrder) const
{
    sorted = *this;
    U64Heap::sort(sorted.item_, sorted.numItems_, reverseOrder);
}
