...
//!
U64Heap::U64Heap(const U64Heap& heap):
Growable(heap)
{
...
//
U64Heap::U64Heap(item_t* item, size_t numItems):
...
//!
U64Heap::U64Heap(unsigned int capacity, int growBy):
Growable(capacity, growBy)
{

    // Allocate all items. Initialize each item when used.
    numItems_ = 0;
    item_ = new item_t[U64Heap::capacity()];
...
//!
const U64Heap& U64Heap::operator =(const U64Heap& heap)
{
...
//!
bool U64Heap::resize(unsigned int newCap)
{
    bool ok;
    if (numItems_ > newCap)
    {
...
//!
bool U64Heap::rmFromIndex(size_t index, item_t& removedItem)
{
...
//!
bool U64Heap::rm(item_t& topItem)
{

    // U64Heap is not empty.
    bool ok;
    if (numItems_ > 0)
    {
        ok = true;
...
//
void U64Heap::heapifyDown(size_t i)
{
    item_t momV;
    for (size_t mom = i;;)
    {
...
//
void U64Heap::heapifyUp(size_t i)
{
    for (size_t kid = i;;)
    {
        item_t momV;
...
//!
void U64Heap::sort(item_t* item, size_t itemCount, bool reverseOrder)
{
