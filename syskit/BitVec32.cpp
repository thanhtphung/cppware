...
const unsigned int BitVec32::INVALID_BIT = 0xffffffffU;


//!
//! Construct a duplicate instance of the given bit vector.
//!
BitVec32::BitVec32(const BitVec32& vec)
{
...
//!
BitVec32::BitVec32(unsigned int maxBits, bool initialVal)
{
    maxBits_ = maxBits;
    rawLength_ = maxBits_? (((maxBits_ - 1) >> 5) + 1): (0);
    raw_ = new word_t[rawLength_];
...
//!
BitVec32::BitVec32(unsigned int maxBits, const word_t* raw, size_t byteSize)
{
    maxBits_ = maxBits;
    rawLength_ = maxBits_? (((maxBits_ - 1) >> 5) + 1): (0);
    raw_ = new word_t[rawLength_];
...
//!
const BitVec32& BitVec32::operator &=(const BitVec32& vec)
{
...
const BitVec32& BitVec32::operator =(const BitVec32& vec)
{
    ...
    if (this != &vec)
    {
        ...
        {
            memcpy(raw_, vec.raw_, rawLength_ * sizeof(*raw_));
            if (maxBits_ & 0x1fU) //keep unused bits clear
            {
                word_t* p = raw_ + (maxBits_ >> 5);
                *p &= ((1U << (maxBits_ & 0x1fU)) - 1);
            }
        }
        else
        {
...
//!
bool BitVec32::clearBit(size_t bit)
{
    bool modified = false;
    if (bit < maxBits_)
    {
...
//!
bool BitVec32::clearBits(size_t loBit, size_t hiBit)
{
    ...
    bool modified = false;
    word_t* p = raw_ + (loBit >> 5);
    word_t bitM = 1U << (loBit & 0x1fU);
    for (;;)
    {
        if (bitM == 1)
        {
        ...
        if (--numBits == 0) break;
        (bitM == 0x80000000U)? (++p, bitM = 1): (bitM <<= 1);
    }
...
//!
bool BitVec32::resize(unsigned int maxBits, bool initialVal)
{

    // Shrinking not allowed.
    bool ok = true;
    unsigned int rawLength = ((maxBits - 1) >> 5) + 1;
    if (maxBits < maxBits_)
    {
        ok = false;
    }
...
//!
bool BitVec32::setBit(size_t bit)
{
    bool modified = false;
    if (bit < maxBits_)
    {
...
//!
bool BitVec32::setBits(size_t loBit, size_t hiBit)
{
...
//!
unsigned int BitVec32::countSetBits() const
{
...
unsigned int BitVec32::doCountSetBits(unsigned int mask)
{
    static bool s_usePopcnt = popcntIsSupported();
...
unsigned int BitVec32::doCountSetBits(unsigned long long mask)
{
    static bool s_usePopcnt = popcntIsSupported();
...
//! firstClearBit().
//!
unsigned int BitVec32::nextClearBit(size_t curBit) const
{
    ...
    curBit = curBit32;
    size_t vecI = curBit >> 5;
    size_t bitI = curBit & 0x1fU;
    const word_t* p = raw_ + vecI;
    if (bitI)
    {
        for (word_t mask = (1U << bitI); mask; mask <<= 1, ++curBit)
        {
            if ((*p & mask) == 0)
            {
    ...
    for (const word_t* pEnd = raw_ + rawLength_; p < pEnd; ++p, curBit += BitsPerWord)
    {
        if (*p != 0xffffffffU)
        {
            for (word_t mask = 0x00000001U; mask; mask <<= 1, ++curBit)
            {
                if ((*p & mask) == 0)
                {
                    return (curBit < maxBits_)? static_cast<unsigned int>(curBit): INVALID_BIT;
...
//!
unsigned int BitVec32::nextSetBit(size_t curBit) const
{
...
//! lastClearBit().
//!
unsigned int BitVec32::prevClearBit(size_t curBit) const
{
    ...
    for (; p >= raw_; --p, curBit -= BitsPerWord)
    {
        if (*p != 0xffffffffU)
        {
            for (word_t mask = 0x80000000U; mask; mask >>= 1, --curBit)
            {
                if ((*p & mask) == 0)
                {
                    return static_cast<unsigned int>(curBit);
...
//! to lastSetBit().
//!
unsigned int BitVec32::prevSetBit(size_t curBit) const
{
    ...
    {
        ulong32_t index;
        if (_BitScanReverse(&index, *p))
        {
            return static_cast<unsigned int>(curBit + index - 31);
        }
    }
...
//!
void BitVec32::clear(size_t loBit, size_t hiBit)
{
...
//!
void BitVec32::set(size_t loBit, size_t hiBit)
{
    size_t numBits = hiBit - loBit + 1;
    word_t* p = raw_ + (loBit >> 5);
    word_t bitM = 1U << (loBit & 0x1fU);
...
//!
//! Construct an unattached BitVec32 iterator.
//!
BitVec32::Itor::Itor()
{
    copyMade_ = false;
...
//!
//! Construct a BitVec32 iterator. Attach iterator to given bit vector.
...
//!
BitVec32::Itor::Itor(const BitVec32& vec, bool makeCopy)
{
    copyMade_ = false;
    vec_ = 0;
...
//!
bool BitVec32::Itor::applyToClearBits(cb0_t cb, void* arg, bool loToHi) const
{
    bool ok = true;
    if (vec_ != 0)
    {
...
//
bool BitVec32::Itor::applyToClearBitsHiToLo(cb0_t cb, void* arg) const
{

    // Last word might have unused bits.
    const word_t* pEnd = vec_->raw_;
    const word_t* p = pEnd + vec_->rawLength_ - 1;
    size_t bit = (vec_->rawLength_ << 5) - 1;
    word_t lastWord = setUnusedBits(*p, vec_->maxBits_);
    if (lastWord != 0xffffffffU)
    {
        for (word_t mask = 0x80000000U; mask; mask >>= 1, --bit)
        {
            if ((lastWord & mask) == 0)
            {
                if (!cb(arg, bit))
...
// each clear bit. Return true if the iterating was not aborted.
//
bool BitVec32::Itor::applyToClearBitsLoToHi(cb0_t cb, void* arg) const
{
    ...
    for (const word_t* pEnd = p + vec_->rawLength_ - 1; p < pEnd; ++p)
    {
        if (*p != 0xffffffffU)
        {
            for (word_t mask = 0x00000001U; mask; mask <<= 1, ++bit)
            {
                if ((*p & mask) == 0)
                {
                    if (!cb(arg, bit))
...
//! true otherwise. Ignore if iterator is unattached.
//!
bool BitVec32::Itor::applyToSetBits(cb0_t cb, void* arg, bool loToHi) const
{
...
//
bool BitVec32::Itor::applyToSetBitsHiToLo(cb0_t cb, void* arg) const
{
...
// each set bit. Return true if the iterating was not aborted.
//
bool BitVec32::Itor::applyToSetBitsLoToHi(cb0_t cb, void* arg) const
{
...
//! bit.
//!
bool BitVec32::Itor::nextClearBit(size_t& bit)
{
...
//! bit.
//!
bool BitVec32::Itor::nextSetBit(size_t& bit)
{
...
//! bit.
//!
bool BitVec32::Itor::prevClearBit(size_t& bit)
{
...
//! bit.
//!
bool BitVec32::Itor::prevSetBit(size_t& bit)
{
...
//!
void BitVec32::Itor::applyToClearBits(cb1_t cb, void* arg, bool loToHi) const
{
    if (vec_ != 0)
    {
...
//
void BitVec32::Itor::applyToClearBitsHiToLo(cb1_t cb, void* arg) const
{
...
// Apply callback to each clear bit.
//
void BitVec32::Itor::applyToClearBitsLoToHi(cb1_t cb, void* arg) const
{
...
//!
void BitVec32::Itor::applyToSetBits(cb1_t cb, void* arg, bool loToHi) const
{
    if (vec_ != 0)
    {
...
//
void BitVec32::Itor::applyToSetBitsHiToLo(cb1_t cb, void* arg) const
{
...
// Apply callback to each set bit.
//
void BitVec32::Itor::applyToSetBitsLoToHi(cb1_t cb, void* arg) const
{
...
//!
void BitVec32::Itor::attach(const BitVec32& vec, bool makeCopy)
{
    ...
    vec_ = makeCopy? new BitVec32(vec): &vec;
    copyMade_ = makeCopy;
