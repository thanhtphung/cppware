...
//!
U32Vec::U32Vec(const U32Vec& vec):
Growable(vec)
{
...
//!
U32Vec::U32Vec(const U32Vec& vec, size_t startAt, size_t itemCount):
Growable(vec)
{
    item_ = new item_t[capacity()];
...
//!
U32Vec::U32Vec(unsigned int capacity, int growBy):
Growable(capacity, growBy)
{

    // Allocate all items. Initialize each item when used.
    item_ = new item_t[U32Vec::capacity()];
    numItems_ = 0;
}
...
//! be dropped.
//!
const U32Vec& U32Vec::operator =(const U32Vec& vec)
{
...
//! further.
//!
U32Vec::item_t* U32Vec::detachRaw()
{
    item_t* raw = item_;
    item_ = 0;
    numItems_ = 0;
...
//!
bool U32Vec::addAtIndex(size_t index, item_t item, bool maintainOrder)
{
...
//!
bool U32Vec::find(item_t item, size_t& foundIndex) const
{
    bool found = false;
...
//!
bool U32Vec::resize(unsigned int newCap)
{
    bool ok;
    if (numItems_ > newCap)
    {
...
//!
bool U32Vec::rm(item_t item, bool maintainOrder)
{
    size_t foundIndex;
...
//!
bool U32Vec::rmFromIndex(size_t index, item_t& removedItem, bool maintainOrder)
{
...
//!
bool U32Vec::search(item_t item, size_t& foundIndex) const
{
...
//!
unsigned int U32Vec::add(size_t count, item_t item)
{
    if (count == 0)
    {
...
//!
unsigned int U32Vec::add(const item_t* raw, size_t itemCount)
{
    if (itemCount == 0)
    {
        unsigned int numAdds = 0;
