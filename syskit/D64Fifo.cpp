...
//!
D64Fifo::D64Fifo(const D64Fifo& q):
Growable(q)
{
...
//!
D64Fifo::D64Fifo(unsigned int capacity, int growBy):
Growable(capacity, growBy)
{
    ...
    item_ = new item_t[D64Fifo::capacity()];
}
...
//! dropped.
//!
const D64Fifo& D64Fifo::operator =(const D64Fifo& q)
{
...
//! Return false otherwise (queue is full).
//!
bool D64Fifo::addAtHead(item_t item)
{
    ...
    if ((numItems_ < capacity()) || grow())
    {
        unsigned int capacity = D64Fifo::capacity();
        if (--head_ >= capacity)
        {
            head_ = capacity - 1;
        }
...
//! return removed item in item). Return false otherwise (queue is empty).
//!
bool D64Fifo::rmFromTail(item_t& item)
{
...
//!
bool D64Fifo::resize(unsigned int newCap)
{
    bool ok;
    unsigned int curCap = capacity();
    if (numItems_ > newCap)
...
//
void D64Fifo::construct(const D64Fifo& q)
{
...
//!
void D64Fifo::Stat::reset(const D64Fifo& q)
{
    numAdds_ = q.numAdds_;
