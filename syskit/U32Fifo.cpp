...
//!
U32Fifo::U32Fifo(const U32Fifo& q):
Growable(q)
{
...
//!
U32Fifo::U32Fifo(unsigned int capacity, int growBy):
Growable(capacity, growBy)
{
    ...
    item_ = new item_t[U32Fifo::capacity()];
}
...
//! dropped.
//!
const U32Fifo& U32Fifo::operator =(const U32Fifo& q)
{
...
//! Return false otherwise (queue is full).
//!
bool U32Fifo::addAtHead(item_t item)
{
    ...
    if ((numItems_ < capacity()) || grow())
    {
        unsigned int capacity = U32Fifo::capacity();
        if (--head_ >= capacity)
        {
            head_ = capacity - 1;
        }
...
//! return removed item in item). Return false otherwise (queue is empty).
//!
bool U32Fifo::rmFromTail(item_t& item)
{
...
//!
bool U32Fifo::resize(unsigned int newCap)
{
    bool ok;
    unsigned int curCap = capacity();
    if (numItems_ > newCap)
...
//
void U32Fifo::construct(const U32Fifo& q)
{
...
//!
void U32Fifo::Stat::reset(const U32Fifo& q)
{
    numAdds_ = q.numAdds_;
