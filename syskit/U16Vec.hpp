 ...
 */
#ifndef SYSKIT_U16_VEC_HPP
#define SYSKIT_U16_VEC_HPP
...
class U16Vec: public Growable
    //!
    //! A class representing an unsorted vector of 16-bit numbers. The vector
...
public:
    ...
    // Constructors.
    U16Vec(const U16Vec& vec);
    U16Vec(const U16Vec& vec, size_t startAt, size_t itemCount);
    U16Vec(unsigned int capacity = DefaultCap, int growBy = 0);

    // Operators.
    const U16Vec& operator =(const U16Vec& vec);
    bool operator !=(const U16Vec& vec) const;
    bool operator ==(const U16Vec& vec) const;
    ...
    unsigned int add(const U16Vec& vec);
    unsigned int add(const U16Vec& vec, size_t startAt, size_t itemCount);
    ...
    unsigned int reset(const U16Vec& vec, size_t startAt, size_t itemCount);
    void reset();
    ...
    void sort(U16Vec& sorted, bool reverseOrder = false) const;
...
inline U16Vec::item_t U16Vec::operator [](size_t index) const
{
    return item_[index];
}
...
//! Peek at given index and return the residing item. Don't do any error
//! checking. Behavior is unpredictable if given index is invalid.
inline U16Vec::item_t& U16Vec::operator [](size_t index)
{
    return item_[index];
}

//! Return true if this vector does not equal given vector.
inline bool U16Vec::operator !=(const U16Vec& vec) const
{
...
//! Peek at given index and return the residing item. Don't do any error
//! checking. Behavior is unpredictable if given index is invalid.
inline U16Vec::item_t U16Vec::peek(size_t index) const
{
    return item_[index];
}
...
//! further.
inline U16Vec::item_t* U16Vec::detachRaw(unsigned int& numItems)
{
    numItems = numItems_;
...
inline bool U16Vec::addIfNotFound(item_t item)
{
    size_t foundIndex;
...
inline bool U16Vec::find(item_t item) const
{
    size_t foundIndex;
...
inline bool U16Vec::rmFromIndex(size_t index, bool maintainOrder)
{
    item_t removedItem;
...
inline bool U16Vec::rmTail(item_t& removedItem)
{
...
inline bool U16Vec::search(item_t item) const
{
    size_t foundIndex;
...
inline bool U16Vec::truncate(size_t numItems)
{
...
inline const U16Vec::item_t* U16Vec::raw() const
{
    return item_;
}
...
inline unsigned int U16Vec::add(const U16Vec& vec)
{
...
inline unsigned int U16Vec::add(const U16Vec& vec, size_t startAt, size_t itemCount)
{
...
inline unsigned int U16Vec::findIndex(item_t item) const
{
    size_t foundIndex;
...
inline unsigned int U16Vec::numItems() const
{
    return numItems_;
}
...
//! grow.
inline unsigned int U16Vec::reset(const U16Vec& vec, size_t startAt, size_t itemCount)
{
    numItems_ = 0;
    unsigned int numAdds = add(vec.item_ + startAt, itemCount);
    return numAdds;
...
inline void U16Vec::setItem(size_t index, item_t item)
{
    item_[index] = item;
}
...
inline void U16Vec::setItem(size_t index, item_t item, item_t& replacedItem)
{
...
inline void U16Vec::sort(bool reverseOrder)
{
    U16Heap::sort(item_, numItems_, reverseOrder);
}
...
inline void U16Vec::sort(U16Vec& sorted, bool reverseOrder) const
{
    sorted = *this;
    U16Heap::sort(sorted.item_, sorted.numItems_, reverseOrder);
}
