 ...
 */
#ifndef SYSKIT_U64_FIFO_HPP
#define SYSKIT_U64_FIFO_HPP
...
//! first-in-first-out queue of 64-bit numbers
class U64Fifo: public Growable
    //!
    //! A class representing a FIFO queue of 64-bit numbers. The queue has
...
public:
    ...
    // Constructors.
    U64Fifo(const U64Fifo& q);
    U64Fifo(unsigned int capacity = DefaultCap, int growBy = 0);

    // Operators.
    const U64Fifo& operator =(const U64Fifo& q);
...
inline U64Fifo::item_t U64Fifo::peek(size_t index) const
{
...
inline bool U64Fifo::addAtHeadCircularly(item_t item)
{
    item_t droppedItem;
...
inline bool U64Fifo::addAtHeadCircularly(item_t item, bool& someWasDropped, item_t& droppedItem)
{
...
//! queue before the given item is added. Return true if successful.
//! Return false otherwise (queue has a zero capacity).
inline bool U64Fifo::addCircularly(item_t item)
{
    item_t droppedItem;
...
//! false otherwise (queue has a zero capacity). This method also
//! provides information on the dropped item, if any.
inline bool U64Fifo::addCircularly(item_t item, bool& someWasDropped, item_t& droppedItem)
{
...
inline unsigned int U64Fifo::numItems() const
{
    return numItems_;
}
...
inline unsigned int U64Fifo::usagePeak() const
{
    return usagePeak_;
}

//! Reset the queue by removing all items.
inline void U64Fifo::reset()
{
    numRms_ += numItems_;
    numItems_ = 0;
    head_ = tail_;
...
//! Get queue stats.
inline U64Fifo::Stat::Stat(const U64Fifo& q)
{
    reset(q);
}
...
inline unsigned int U64Fifo::Stat::numFails() const
{
    return numFails_;
}
...
//! Return the usage peak.
//! This is high of the number of items in the queue.
inline unsigned int U64Fifo::Stat::usagePeak() const
{
    return usagePeak_;
}
...
inline unsigned long long U64Fifo::Stat::numAdds() const
{
    return numAdds_;
}
...
inline unsigned long long U64Fifo::Stat::numRms() const
{
    return numRms_;
}
