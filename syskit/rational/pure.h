...
//
// Functions to track memory blocks created via COM or custom allocators
//
#define	PURIFY_TRACK_AS_HEAP				0x00000001	// The block is allocated within a Windows heap
#define	PURIFY_TRACK_NO_REDZONE				0x00000002	// The block should not be padded with ABW detection zones
#define	PURIFY_TRACK_AS_INITIALIZED			0x00000004	// The block should be considered initialized
size_t __cdecl PurifyPreAlloc(size_t cbRequest, unsigned int dwFlags);
void * __cdecl PurifyPostAlloc(void *pActual, unsigned int dwFlags);
void * __cdecl PurifyPreFree(void *pRequest, unsigned int dwFlags);
void   __cdecl PurifyPostFree(unsigned int dwFlags);
size_t __cdecl PurifyPreRealloc(void *pRequest, size_t cbRequest, void **ppNewRequest, unsigned int dwFlags);
void * __cdecl PurifyPostRealloc(void *pActual, unsigned int dwFlags);
void * __cdecl PurifyPreGetSize(void *pRequest, unsigned int dwFlags);
size_t __cdecl PurifyPostGetSize(size_t cbActual, unsigned int dwFlags);
void * __cdecl PurifyPreDidAlloc(void *pRequest, unsigned int dwFlags);
int    __cdecl PurifyPostDidAlloc(void *pRequest, unsigned int dwFlags, int fActual);
void   __cdecl PurifyPreHeapMinimize();
void   __cdecl PurifyPostHeapMinimize();
//
...
void __cdecl StartPureRTDebugLogging(void)	;
void __cdecl StopPureRTDebugLogging(void)	;
