...
//!
D64Vec::D64Vec(const D64Vec& vec):
Growable(vec)
{
...
//!
D64Vec::D64Vec(const D64Vec& vec, size_t startAt, size_t itemCount):
Growable(vec)
{
    item_ = new item_t[capacity()];
...
//!
D64Vec::D64Vec(unsigned int capacity, int growBy):
Growable(capacity, growBy)
{

    // Allocate all items. Initialize each item when used.
    item_ = new item_t[D64Vec::capacity()];
    numItems_ = 0;
}
...
//!
bool D64Vec::addAtIndex(size_t index, item_t item, bool maintainOrder)
{
...
//!
bool D64Vec::find(item_t item, size_t& foundIndex) const
{
    bool found = false;
...
//!
bool D64Vec::resize(unsigned int newCap)
{
    bool ok;
    if (numItems_ > newCap)
    {
...
//!
bool D64Vec::rm(item_t item, bool maintainOrder)
{
    size_t foundIndex;
...
//!
bool D64Vec::rmFromIndex(size_t index, item_t& removedItem, bool maintainOrder)
{
...
//!
bool D64Vec::search(item_t item, size_t& foundIndex) const
{
