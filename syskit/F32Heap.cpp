...
//!
F32Heap::F32Heap(const F32Heap& heap):
Growable(heap)
{
...
//
F32Heap::F32Heap(item_t* item, size_t numItems):
...
//!
F32Heap::F32Heap(unsigned int capacity, int growBy):
Growable(capacity, growBy)
{

    // Allocate all items. Initialize each item when used.
    numItems_ = 0;
    item_ = new item_t[F32Heap::capacity()];
...
//!
const F32Heap& F32Heap::operator =(const F32Heap& heap)
{
...
//!
bool F32Heap::resize(unsigned int newCap)
{
    bool ok;
    if (numItems_ > newCap)
    {
...
//!
bool F32Heap::rmFromIndex(size_t index, item_t& removedItem)
{
...
//!
bool F32Heap::rm(item_t& topItem)
{

    // F32Heap is not empty.
    bool ok;
    if (numItems_ > 0)
    {
        ok = true;
...
//
void F32Heap::heapifyDown(size_t i)
{
    item_t momV;
    for (size_t mom = i;;)
    {
...
//
void F32Heap::heapifyUp(size_t i)
{
    for (size_t kid = i;;)
    {
        item_t momV;
...
//!
void F32Heap::sort(item_t* item, size_t itemCount, bool reverseOrder)
{
