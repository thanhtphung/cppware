 ...
 */
#ifndef SYSKIT_U32_VEC_HPP
#define SYSKIT_U32_VEC_HPP
...
//! vector of 32-bit numbers
class U32Vec: public Growable
    //!
    //! A class representing an unsorted vector of 32-bit numbers. The vector
...
public:
    ...
    // Constructors.
    U32Vec(const U32Vec& vec);
    U32Vec(const U32Vec& vec, size_t startAt, size_t itemCount);
    U32Vec(unsigned int capacity = DefaultCap, int growBy = 0);

    // Operators.
    const U32Vec& operator =(const U32Vec& vec);
    bool operator !=(const U32Vec& vec) const;
    bool operator ==(const U32Vec& vec) const;
    ...
    unsigned int add(const U32Vec& vec);
    unsigned int add(const U32Vec& vec, size_t startAt, size_t itemCount);
    ...
    unsigned int reset(const U32Vec& vec, size_t startAt, size_t itemCount);
    void reset();
    ...
    void sort(U32Vec& sorted, bool reverseOrder = false) const;
...
inline U32Vec::item_t U32Vec::operator [](size_t index) const
{
    return item_[index];
}
...
//! Peek at given index and return the residing item. Don't do any error
//! checking. Behavior is unpredictable if given index is invalid.
inline U32Vec::item_t& U32Vec::operator [](size_t index)
{
    return item_[index];
}

//! Return true if this vector does not equal given vector.
inline bool U32Vec::operator !=(const U32Vec& vec) const
{
...
//! Peek at given index and return the residing item. Don't do any error
//! checking. Behavior is unpredictable if given index is invalid.
inline U32Vec::item_t U32Vec::peek(size_t index) const
{
    return item_[index];
}
...
//! further.
inline U32Vec::item_t* U32Vec::detachRaw(unsigned int& numItems)
{
    numItems = numItems_;
...
inline bool U32Vec::addIfNotFound(item_t item)
{
    size_t foundIndex;
...
inline bool U32Vec::find(item_t item) const
{
    size_t foundIndex;
...
inline bool U32Vec::rmFromIndex(size_t index, bool maintainOrder)
{
    item_t removedItem;
...
inline bool U32Vec::rmTail(item_t& removedItem)
{
...
inline bool U32Vec::search(item_t item) const
{
    size_t foundIndex;
...
inline bool U32Vec::truncate(size_t numItems)
{
...
inline const U32Vec::item_t* U32Vec::raw() const
{
    return item_;
}
...
inline unsigned int U32Vec::add(const U32Vec& vec)
{
...
inline unsigned int U32Vec::add(const U32Vec& vec, size_t startAt, size_t itemCount)
{
...
inline unsigned int U32Vec::findIndex(item_t item) const
{
    size_t foundIndex;
...
inline unsigned int U32Vec::numItems() const
{
    return numItems_;
}
...
//! grow.
inline unsigned int U32Vec::reset(const U32Vec& vec, size_t startAt, size_t itemCount)
{
    numItems_ = 0;
    unsigned int numAdds = add(vec.item_ + startAt, itemCount);
    return numAdds;
...
inline void U32Vec::setItem(size_t index, item_t item)
{
    item_[index] = item;
}
...
inline void U32Vec::setItem(size_t index, item_t item, item_t& replacedItem)
{
...
inline void U32Vec::sort(bool reverseOrder)
{
    U32Heap::sort(item_, numItems_, reverseOrder);
}
...
inline void U32Vec::sort(U32Vec& sorted, bool reverseOrder) const
{
    sorted = *this;
    U32Heap::sort(sorted.item_, sorted.numItems_, reverseOrder);
}
