...
//!
U32Heap::U32Heap(const U32Heap& heap):
Growable(heap)
{
...
//
U32Heap::U32Heap(item_t* item, size_t numItems):
...
//!
U32Heap::U32Heap(unsigned int capacity, int growBy):
Growable(capacity, growBy)
{

    // Allocate all items. Initialize each item when used.
    numItems_ = 0;
    item_ = new item_t[U32Heap::capacity()];
...
//!
const U32Heap& U32Heap::operator =(const U32Heap& heap)
{
...
//!
bool U32Heap::resize(unsigned int newCap)
{
    bool ok;
    if (numItems_ > newCap)
    {
...
//!
bool U32Heap::rmFromIndex(size_t index, item_t& removedItem)
{
...
//!
bool U32Heap::rm(item_t& topItem)
{

    // U32Heap is not empty.
    bool ok;
    if (numItems_ > 0)
    {
        ok = true;
...
//
void U32Heap::heapifyDown(size_t i)
{
    item_t momV;
    for (size_t mom = i;;)
    {
...
//
void U32Heap::heapifyUp(size_t i)
{
    for (size_t kid = i;;)
    {
        item_t momV;
...
//!
void U32Heap::sort(item_t* item, size_t itemCount, bool reverseOrder)
{
