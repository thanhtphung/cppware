...
//!
D64Heap::D64Heap(const D64Heap& heap):
Growable(heap)
{
...
//
D64Heap::D64Heap(item_t* item, size_t numItems):
...
//!
D64Heap::D64Heap(unsigned int capacity, int growBy):
Growable(capacity, growBy)
{

    // Allocate all items. Initialize each item when used.
    numItems_ = 0;
    item_ = new item_t[D64Heap::capacity()];
...
//!
const D64Heap& D64Heap::operator =(const D64Heap& heap)
{
...
//!
bool D64Heap::resize(unsigned int newCap)
{
    bool ok;
    if (numItems_ > newCap)
    {
...
//!
bool D64Heap::rmFromIndex(size_t index, item_t& removedItem)
{
...
//!
bool D64Heap::rm(item_t& topItem)
{

    // D64Heap is not empty.
    bool ok;
    if (numItems_ > 0)
    {
        ok = true;
...
//
void D64Heap::heapifyDown(size_t i)
{
    item_t momV;
    for (size_t mom = i;;)
    {
...
//
void D64Heap::heapifyUp(size_t i)
{
    for (size_t kid = i;;)
    {
        item_t momV;
...
//!
void D64Heap::sort(item_t* item, size_t itemCount, bool reverseOrder)
{
