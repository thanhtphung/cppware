 ...
 */
#ifndef SYSKIT_F32_VEC_HPP
#define SYSKIT_F32_VEC_HPP
...
class F32Vec: public Growable
    //!
    //! A class representing an unsorted vector of 32-bit floats. The vector
...
public:
    ...
    // Constructors.
    F32Vec(const F32Vec& vec);
    F32Vec(const F32Vec& vec, size_t startAt, size_t itemCount);
    F32Vec(unsigned int capacity = DefaultCap, int growBy = 0);

    // Operators.
    const F32Vec& operator =(const F32Vec& vec);
    bool operator !=(const F32Vec& vec) const;
    bool operator ==(const F32Vec& vec) const;
    ...
    unsigned int add(const F32Vec& vec);
    unsigned int add(const F32Vec& vec, size_t startAt, size_t itemCount);
    ...
    unsigned int reset(const F32Vec& vec, size_t startAt, size_t itemCount);
    void reset();
    ...
    void sort(F32Vec& sorted, bool reverseOrder = false) const;
...
inline F32Vec::item_t F32Vec::operator [](size_t index) const
{
    return item_[index];
}
...
//! Peek at given index and return the residing item. Don't do any error
//! checking. Behavior is unpredictable if given index is invalid.
inline F32Vec::item_t& F32Vec::operator [](size_t index)
{
    return item_[index];
}

//! Return true if this vector does not equal given vector.
inline bool F32Vec::operator !=(const F32Vec& vec) const
{
...
//! Peek at given index and return the residing item. Don't do any error
//! checking. Behavior is unpredictable if given index is invalid.
inline F32Vec::item_t F32Vec::peek(size_t index) const
{
    return item_[index];
}
...
//! further.
inline F32Vec::item_t* F32Vec::detachRaw(unsigned int& numItems)
{
    numItems = numItems_;
...
inline bool F32Vec::addIfNotFound(item_t item)
{
    size_t foundIndex;
...
inline bool F32Vec::find(item_t item) const
{
    size_t foundIndex;
...
inline bool F32Vec::rmFromIndex(size_t index, bool maintainOrder)
{
    item_t removedItem;
...
inline bool F32Vec::rmTail(item_t& removedItem)
{
...
inline bool F32Vec::search(item_t item) const
{
    size_t foundIndex;
...
inline bool F32Vec::truncate(size_t numItems)
{
...
inline const F32Vec::item_t* F32Vec::raw() const
{
    return item_;
}
...
inline unsigned int F32Vec::add(const F32Vec& vec)
{
...
inline unsigned int F32Vec::add(const F32Vec& vec, size_t startAt, size_t itemCount)
{
...
inline unsigned int F32Vec::findIndex(item_t item) const
{
    size_t foundIndex;
...
inline unsigned int F32Vec::numItems() const
{
    return numItems_;
}
...
//! grow.
inline unsigned int F32Vec::reset(const F32Vec& vec, size_t startAt, size_t itemCount)
{
    numItems_ = 0;
    unsigned int numAdds = add(vec.item_ + startAt, itemCount);
    return numAdds;
...
inline void F32Vec::setItem(size_t index, item_t item)
{
    item_[index] = item;
}
...
inline void F32Vec::setItem(size_t index, item_t item, item_t& replacedItem)
{
...
inline void F32Vec::sort(bool reverseOrder)
{
    F32Heap::sort(item_, numItems_, reverseOrder);
}
...
inline void F32Vec::sort(F32Vec& sorted, bool reverseOrder) const
{
    sorted = *this;
    F32Heap::sort(sorted.item_, sorted.numItems_, reverseOrder);
}
