...
//!
Fifo::Fifo(unsigned int capacity, int growBy):
Growable(capacity, growBy)
{
    ...
    item_ = new item_t[Fifo::capacity()];
}
...
//! dropped.
//!
const Fifo& Fifo::operator =(const Fifo& q)
{
...
//! Return false otherwise (queue is full).
//!
bool Fifo::addAtHead(item_t item)
{
    ...
    if ((numItems_ < capacity()) || grow())
    {
        unsigned int capacity = Fifo::capacity();
        if (--head_ >= capacity)
        {
            head_ = capacity - 1;
        }
...
//! return removed item in item). Return false otherwise (queue is empty).
//!
bool Fifo::rmFromTail(item_t& item)
{
...
//!
bool Fifo::resize(unsigned int newCap)
{
    bool ok;
    unsigned int curCap = capacity();
    if (numItems_ > newCap)
...
//
void Fifo::construct(const Fifo& q)
{
...
//!
void Fifo::Stat::reset(const Fifo& q)
{
    numAdds_ = q.numAdds_;
