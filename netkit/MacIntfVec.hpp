 ...
 */
#ifndef NETKIT_MAC_INTF_VEC_HPP
#define NETKIT_MAC_INTF_VEC_HPP
...
//! vector of MacIntf instances
class MacIntfVec: private syskit::U64Vec
{

public:
    typedef MacIntf item_t;
    ...
    // Constructors.
    MacIntfVec(const MacIntfVec& vec);
    MacIntfVec(unsigned int capacity = DefaultCap, int growBy = -1);

    // Operators.
    bool operator !=(const MacIntfVec& vec) const;
    bool operator ==(const MacIntfVec& vec) const;
    const MacIntfVec& operator =(const MacIntfVec& vec);
    ...
    unsigned int add(const MacIntfVec& vec);
    unsigned int add(const MacIntfVec& vec, size_t startAt, size_t itemCount);
    unsigned int add(size_t count, const item_t& item);
    unsigned int reset(const MacIntfVec& vec, size_t startAt, size_t itemCount);
    void reset();
    ...
    void sort(MacIntfVec& sorted, bool reverseOrder = false) const;
...
inline bool MacIntfVec::operator !=(const MacIntfVec& vec) const
{
...
//! be dropped.
inline const MacIntfVec& MacIntfVec::operator =(const MacIntfVec& vec)
{
    U64Vec::operator =(vec);
    return *this;
}
...
inline const MacIntfVec::item_t& MacIntfVec::operator [](size_t index) const
{
...
inline bool MacIntfVec::rmFromIndex(size_t index, bool maintainOrder)
{
...
//! move up to replace the removed item.
inline bool MacIntfVec::rmFromIndex(size_t index, item_t& removedItem, bool maintainOrder)
{
...
inline bool MacIntfVec::rmTail(item_t& removedItem)
{
    U64Vec::item_t item64;
...
//! Peek at given index and return the residing item. Don't do any error
//! checking. Behavior is unpredictable if given index is invalid.
inline const MacIntfVec::item_t& MacIntfVec::peek(size_t index) const
{
    const item_t& item = *reinterpret_cast<const item_t*>(U64Vec::raw() + index);
    return item;
}
...
inline unsigned int MacIntfVec::add(const MacIntfVec& vec)
{
...
inline unsigned int MacIntfVec::add(const MacIntfVec& vec, size_t startAt, size_t itemCount)
{
...
inline unsigned int MacIntfVec::add(size_t count, const item_t& item)
{
...
//! grow.
inline unsigned int MacIntfVec::reset(const MacIntfVec& vec, size_t startAt, size_t itemCount)
{
...
inline void MacIntfVec::setItem(size_t index, const item_t& item)
{
...
inline void MacIntfVec::setItem(size_t index, const item_t& item, item_t& replacedItem)
{
    U64Vec::item_t item64;
...
inline void MacIntfVec::sort(MacIntfVec& sorted, bool reverseOrder) const
{
