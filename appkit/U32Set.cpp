...
//!
U32Set::U32Set(const U32Set& set):
Set(set)
{
...
//! used.
//!
U32Set::U32Set(const char* s, char delim, key_t validMin, key_t validMax, unsigned int capacity):
Set(TYPE, capacity)
{
...
//!
U32Set::U32Set(const char* s, key_t validMin, key_t validMax, unsigned int capacity):
Set(TYPE, capacity)
{
    construct(validMin, validMax);
    add(s, ',');
...
//!
U32Set::U32Set(key_t validMin, key_t validMax, unsigned int capacity):
Set(TYPE, capacity)
{
    construct(validMin, validMax);
}
...
//!
bool U32Set::operator ==(const U32Set& set) const
{
...
//!
const U32Set& U32Set::operator &=(const U32Set& set)
{
...
//!
Set* U32Set::clone() const
{
    return new U32Set(*this);
}
...
//!\endcode
//!
String U32Set::toString(const char* delim) const
{
...
//!
U32Set::compareResult_e U32Set::compare(const Set& set) const
{
    ...
    compareResult_e rc;
    const U32Set* that = (const U32Set*)(&set);
    if (numRanges_ > 0)
    {
...
//!
bool U32Set::add(const U32Set& set)
{
...
//!
bool U32Set::add(const char* s, char delim)
{
...
//!
bool U32Set::add(key_t loKey, key_t hiKey)
{
...
// successful.
//
bool U32Set::addAt(size_t i, key_t loKey, key_t hiKey)
{
...
//
bool U32Set::addNear(size_t i, key_t loKey, key_t hiKey)
{
...
//!
bool U32Set::contains(const U32Set& set) const
{
    bool answer = true;
    if (*this == set)
    {
...
//!
bool U32Set::contains(const char* s, char delim) const
{
...
//
bool U32Set::contains(key_t loKey, key_t hiKey, unsigned int& foundIndex) const
{
...
//!
//! See if string s is a valid unsigned integer set specification.
...
//! used.
//!
bool U32Set::isValid(const char* s, char delim, bool beStrict) const
{
...
//!
bool U32Set::overlaps(const U32Set& set) const
{
...
//!
bool U32Set::pick(key_t& removedKey)
{
    bool ok;
    if (numRanges_ == 0)
    {
...
//!
bool U32Set::resize(unsigned int newCap)
{
    bool ok;
...
//! from 1 to 49.
//!
bool U32Set::rm(const U32Set& set)
{
    keyCount_t oldNumKeys = numKeys_;
...
//! will be used.
//!
bool U32Set::rm(const char* s, char delim)
{
...
//! 49.
//!
bool U32Set::rm(key_t loKey, key_t hiKey)
{
    keyCount_t oldNumKeys = numKeys_;
...
//
bool U32Set::rmAt(size_t i, key_t loKey, key_t hiKey)
{
...
// Return true if successful.
//
bool U32Set::rmNear(size_t i, key_t loKey, key_t hiKey)
{
...
//!
bool U32Set::toKey(const char* s, key_t& key) const
{
    char* p;
...
//! this set.
//!
bool U32Set::toRange(const char* s, key_t& loKey, key_t& hiKey, bool beStrict) const
{
...
//! if *item0 > *item1.
//!
int U32Set::compareP(const void* item0, const void* item1)
{

    int rc;
    const U32Set* k0 = static_cast<const U32Set*>(item0);
    const U32Set* k1 = static_cast<const U32Set*>(item1);
...
//
void U32Set::construct(key_t validMin, key_t validMax)
{
    numKeys_ = 0;
    numRanges_ = 0;
...
//
void U32Set::rmOverlaps(size_t i)
{
...
//!
//! Construct an unattached U32Set iterator.
//!
U32Set::Itor::Itor()
{
    copyMade_ = false;
...
//!
//! Construct a U32Set iterator. Attach iterator to given set.
...
//!
U32Set::Itor::Itor(const U32Set& set, bool makeCopy)
{
    copyMade_ = false;
    set_ = 0;
...
//! is unattached.
//!
bool U32Set::Itor::applyHiToLo(keyCb0_t cb, void* arg) const
{

    // Ignore if iterator is unattached.
    if (set_ != 0)
    {
        const U32Set::range_t* p0 = set_->rangeVec_;
        const U32Set::range_t* p = p0 + set_->numRanges_ - 1;
        for (; p >= p0; --p)
        {
...
//! is unattached.
//!
bool U32Set::Itor::applyHiToLo(rangeCb0_t cb, void* arg) const
{
...
//! is unattached.
//!
bool U32Set::Itor::applyLoToHi(keyCb0_t cb, void* arg) const
{

    // Ignore if iterator is unattached.
    if (set_ != 0)
    {
        const U32Set::range_t* p = set_->rangeVec_;
        const U32Set::range_t* pEnd = p + set_->numRanges_;
        for (; p < pEnd; ++p)
        {
...
//! is unattached.
//!
bool U32Set::Itor::applyLoToHi(rangeCb0_t cb, void* arg) const
{
...
//!
bool U32Set::Itor::curRange(key_t& loKey, key_t& hiKey) const
{
    bool ok;
...
//! the first key.
//!
bool U32Set::Itor::nextKey(key_t& key)
{
...
//! the first range.
//!
bool U32Set::Itor::nextRange(key_t& loKey, key_t& hiKey)
{
    ...
    if (curIndex_ == INVALID_INDEX)
    {
        if (set_->numRanges_ > 0)
        {
            ok = true;
            curIndex_ = 0;
            const U32Set::range_t& r = set_->rangeVec_[curIndex_];
            loKey = r.loKey;
            hiKey = r.hiKey;
            curKey_ = loKey;
        }
...
//! the last key.
//!
bool U32Set::Itor::prevKey(key_t& key)
{
...
//! the last range.
//!
bool U32Set::Itor::prevRange(key_t& loKey, key_t& hiKey)
{
...
//! Ignore if iterator is unattached.
//!
void U32Set::Itor::applyHiToLo(keyCb1_t cb, void* arg) const
{
...
//! Ignore if iterator is unattached.
//!
void U32Set::Itor::applyHiToLo(rangeCb1_t cb, void* arg) const
{
...
//! Ignore if iterator is unattached.
//!
void U32Set::Itor::applyLoToHi(keyCb1_t cb, void* arg) const
{
...
//! Ignore if iterator is unattached.
//!
void U32Set::Itor::applyLoToHi(rangeCb1_t cb, void* arg) const
{
...
//!
void U32Set::Itor::attach(const U32Set& set, bool makeCopy)
{
    ...
    set_ = makeCopy? new U32Set(set): &set;
    copyMade_ = makeCopy;
