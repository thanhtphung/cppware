 ...
 */
#ifndef APPKIT_U32_SET_HPP
#define APPKIT_U32_SET_HPP
...
//! set of unsigned 32-bit numbers
class U32Set: public Set
    //!
    //! A class representing a set of unsigned integer keys. Implemented
    //! as an array of contiguous ranges. A U32Set instance has type
    //! "U32" if type() is used. A U32Set instance consists of zero
    //! or more valid unsigned integer keys. Valid unsigned integer keys
...
public:
    ...
    // Constructors.
    U32Set(const U32Set& set);
    U32Set(const char* s, char delim, key_t validMin = VALID_MIN, key_t validMax = VALID_MAX, unsigned int capacity = DefaultCap);
    U32Set(const char* s, key_t validMin = VALID_MIN, key_t validMax = VALID_MAX, unsigned int capacity = DefaultCap);
    U32Set(key_t validMin = VALID_MIN, key_t validMax = VALID_MAX, unsigned int capacity = DefaultCap);

    // Operators.
    bool operator !=(const U32Set& set) const;
    bool operator ==(const U32Set& set) const;
    const U32Set& operator &=(const U32Set& set);
    const U32Set& operator =(const U32Set& set);
    const U32Set& operator |=(const U32Set& set);

    // Set queries.
    bool contains(const U32Set& set) const;
    bool contains(const char* s, char delim = ',') const;
    bool contains(key_t key) const;
    bool contains(key_t loKey, key_t hiKey) const;
    bool overlaps(const U32Set& set) const;
    key_t maxKey() const;
    key_t minKey() const;
    ...
    class Itor
        //!
        //! A class representing a U32Set iterator. It provides a scheme to
        //! traverse the keys in a U32Set instance. Example:
        //!\code
        //! U32Set::Itor it(set);
        //! U32Set::key_t loKey;
        //! U32Set::key_t hiKey;
    ...
    public:
        ...
        Itor();
        Itor(const U32Set& set, bool makeCopy = false);
        ~Itor();
        ...
        // Utilities.
        const U32Set* set() const;
        void attach(const U32Set& set, bool makeCopy = false);
        void detach();
...
inline bool U32Set::operator !=(const U32Set& set) const
{
    return !(operator ==(set));
}
...
inline U32Set::key_t U32Set::hiKeyAt(size_t index) const
{
...
//! Behavior is unpredictable if given index is invalid.
inline U32Set::key_t U32Set::loKeyAt(size_t index) const
{
...
inline U32Set::key_t U32Set::maxKey() const
{
...
inline bool U32Set::contains(key_t loKey, key_t hiKey) const
{
    unsigned int foundIndex;
...
inline unsigned int U32Set::numRanges() const
{
    return numRanges_;
}
...
//! Behavior is unpredictable if given index is invalid.
inline void U32Set::getRange(size_t index, key_t& loKey, key_t& hiKey) const
{
...
inline bool U32Set::Itor::curKey(key_t& key) const
{
...
inline const U32Set* U32Set::Itor::set() const
{
    return set_;
}
