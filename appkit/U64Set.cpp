...
const U64Set::key_t U64Set::VALID_MAX = 0xfffffffffffffffeULL;
const U64Set::key_t U64Set::VALID_MIN = 0;


//!
//! Construct a duplicate instance of the given set.
//!
U64Set::U64Set(const U64Set& set):
Set(set)
{
...
//! used.
//!
U64Set::U64Set(const char* s, char delim, key_t validMin, key_t validMax, unsigned int capacity):
Set(TYPE, capacity)
{
...
//!
U64Set::U64Set(const char* s, key_t validMin, key_t validMax, unsigned int capacity):
Set(TYPE, capacity)
{
    construct(validMin, validMax);
    add(s, ',');
...
//!
U64Set::U64Set(key_t validMin, key_t validMax, unsigned int capacity):
Set(TYPE, capacity)
{
    construct(validMin, validMax);
}
...
//!
bool U64Set::operator ==(const U64Set& set) const
{
...
//!
const U64Set& U64Set::operator &=(const U64Set& set)
{
...
//!
Set* U64Set::clone() const
{
    return new U64Set(*this);
}
...
//!\endcode
//!
String U64Set::toString(const char* delim) const
{
    char buf[1 + U64::MaxDigits];
    buf[0] = '-';
    ...
    {
        size_t n = U64::toDigits(p->loKey, buf + 1);
        s.append(buf + 1, n);
        if (p->loKey != p->hiKey)
        {
            n = U64::toDigits(p->hiKey, buf + 1);
            s.append(buf, n + 1);
        }
    }
...
//!
U64Set::compareResult_e U64Set::compare(const Set& set) const
{
    ...
    compareResult_e rc;
    const U64Set* that = (const U64Set*)(&set);
    if (numRanges_ > 0)
    {
...
//!
bool U64Set::add(const U64Set& set)
{
...
//!
bool U64Set::add(const char* s, char delim)
{
...
//!
bool U64Set::add(key_t loKey, key_t hiKey)
{
...
// successful.
//
bool U64Set::addAt(size_t i, key_t loKey, key_t hiKey)
{
...
//
bool U64Set::addNear(size_t i, key_t loKey, key_t hiKey)
{
    ...
    {
        keyCount_t oldNumKeys = r.hiKey - r.loKey + 1ULL;
        if (r.loKey > loKey)
        {
            r.loKey = loKey;
        }
        ...
        numKeys_ += (r.hiKey - r.loKey + 1ULL) - oldNumKeys;
    }
...
//!
bool U64Set::contains(const U64Set& set) const
{
    bool answer = true;
    if (*this == set)
    {
...
//!
bool U64Set::contains(const char* s, char delim) const
{
...
//
bool U64Set::contains(key_t loKey, key_t hiKey, unsigned int& foundIndex) const
{
...
//!
//! See if string s is a valid 64-bit number set specification.
...
//! used.
//!
bool U64Set::isValid(const char* s, char delim, bool beStrict) const
{
...
//!
bool U64Set::overlaps(const U64Set& set) const
{
...
//!
bool U64Set::pick(key_t& removedKey)
{
    bool ok;
    if (numRanges_ == 0)
    {
    ...
    else
    {
        ...
        const range_t& r = rangeVec_[i];
        key_t key = r.loKey + static_cast<key_t>((rand64 >> 3) % (r.hiKey - r.loKey + 1ULL));
        ok = rmAt(i, key, key);
        if (ok)
        {
            removedKey = key;
...
//!
bool U64Set::resize(unsigned int newCap)
{
    bool ok;
...
//! from 1 to 49.
//!
bool U64Set::rm(const U64Set& set)
{
    keyCount_t oldNumKeys = numKeys_;
...
//! will be used.
//!
bool U64Set::rm(const char* s, char delim)
{
...
//! 49.
//!
bool U64Set::rm(key_t loKey, key_t hiKey)
{
    keyCount_t oldNumKeys = numKeys_;
...
//
bool U64Set::rmAt(size_t i, key_t loKey, key_t hiKey)
{

    // Assume ok.
    bool ok = true;
    range_t& r = rangeVec_[i];
    keyCount_t numKeys = r.hiKey - r.loKey + 1ULL;
    ...
    if (loKey <= r.loKey)
    {
        ...
        {
            r.loKey = hiKey + 1;
            numKeys_ += (r.hiKey - r.loKey + 1ULL) - numKeys;
        }
    }
    ...
    else
    {
        if (hiKey < r.hiKey)
        {
            ...
            r1.hiKey = loKey - 1;
            numKeys_ += (r1.hiKey - r1.loKey + 1ULL) - numKeys;
        }
        else
        {
            r.hiKey = loKey - 1;
...
// Return true if successful.
//
bool U64Set::rmNear(size_t i, key_t loKey, key_t hiKey)
{
...
//!
bool U64Set::toKey(const char* s, key_t& key) const
{
    char* p;
    unsigned long long k = strtoull(s, &p, 0);
...
//! this set.
//!
bool U64Set::toRange(const char* s, key_t& loKey, key_t& hiKey, bool beStrict) const
{
    ...
    char* p;
    unsigned long long lo = strtoull(s, &p, 0);
    if (S32::isDigit(*s) && (p != s) && (*p == '-'))
    {
        s = p + 1;
        unsigned long long hi = strtoull(s, &p, 0);
...
//! if *item0 > *item1.
//!
int U64Set::compareP(const void* item0, const void* item1)
{
    int rc;
    const U64Set* k0 = static_cast<const U64Set*>(item0);
    const U64Set* k1 = static_cast<const U64Set*>(item1);
...
//
void U64Set::construct(key_t validMin, key_t validMax)
{
    numKeys_ = 0;
    numRanges_ = 0;
...
//
void U64Set::rmOverlaps(size_t i)
{
...
//!
//! Construct an unattached U64Set iterator.
//!
U64Set::Itor::Itor()
{
    copyMade_ = false;
...
//!
//! Construct a U64Set iterator. Attach iterator to given set.
...
//!
U64Set::Itor::Itor(const U64Set& set, bool makeCopy)
{
    copyMade_ = false;
    set_ = 0;
...
//! is unattached.
//!
bool U64Set::Itor::applyHiToLo(keyCb0_t cb, void* arg) const
{

    // Ignore if iterator is unattached.
    if (set_ != 0)
    {
        const U64Set::range_t* p0 = set_->rangeVec_;
        const U64Set::range_t* p = p0 + set_->numRanges_ - 1;
        for (; p >= p0; --p)
        {
...
//! is unattached.
//!
bool U64Set::Itor::applyHiToLo(rangeCb0_t cb, void* arg) const
{
...
//! is unattached.
//!
bool U64Set::Itor::applyLoToHi(keyCb0_t cb, void* arg) const
{

    // Ignore if iterator is unattached.
    if (set_ != 0)
    {
        const U64Set::range_t* p = set_->rangeVec_;
        const U64Set::range_t* pEnd = p + set_->numRanges_;
        for (; p < pEnd; ++p)
        {
...
//! is unattached.
//!
bool U64Set::Itor::applyLoToHi(rangeCb0_t cb, void* arg) const
{
...
//!
bool U64Set::Itor::curRange(key_t& loKey, key_t& hiKey) const
{
    bool ok;
...
//! the first key.
//!
bool U64Set::Itor::nextKey(key_t& key)
{
...
//! the first range.
//!
bool U64Set::Itor::nextRange(key_t& loKey, key_t& hiKey)
{
    ...
    if (curIndex_ == INVALID_INDEX)
    {
        if (set_->numRanges_ > 0)
        {
            ok = true;
            curIndex_ = 0;
            const U64Set::range_t& r = set_->rangeVec_[curIndex_];
            loKey = r.loKey;
            hiKey = r.hiKey;
            curKey_ = loKey;
        }
...
//! the last key.
//!
bool U64Set::Itor::prevKey(key_t& key)
{
...
//! the last range.
//!
bool U64Set::Itor::prevRange(key_t& loKey, key_t& hiKey)
{
...
//! Ignore if iterator is unattached.
//!
void U64Set::Itor::applyHiToLo(keyCb1_t cb, void* arg) const
{
...
//! Ignore if iterator is unattached.
//!
void U64Set::Itor::applyHiToLo(rangeCb1_t cb, void* arg) const
{
...
//! Ignore if iterator is unattached.
//!
void U64Set::Itor::applyLoToHi(keyCb1_t cb, void* arg) const
{
...
//! Ignore if iterator is unattached.
//!
void U64Set::Itor::applyLoToHi(rangeCb1_t cb, void* arg) const
{
...
//!
void U64Set::Itor::attach(const U64Set& set, bool makeCopy)
{
    ...
    set_ = makeCopy? new U64Set(set): &set;
    copyMade_ = makeCopy;
