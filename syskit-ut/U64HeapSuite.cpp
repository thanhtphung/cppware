...
const char ITEMS[] = "aRandomStringUsedForU64HeapPopulation!!!";


U64HeapSuite::U64HeapSuite()
{
}
...
void U64HeapSuite::testAdd00()
{
    U64Heap heap0(256 /*capacity*/, 0 /*growBy*/);
    for (const char* p = ITEMS; *p != 0; heap0.add(*p++));
    U64Heap heap1(256 /*capacity*/, 0 /*growBy*/);
    for (const char* p = ITEMS; *p != 0; heap1.add(*p++));
    U64Heap heap2(2 /*capacity*/, -1 /*growBy*/);
    for (const char* p = ITEMS; *p != 0; heap2.add(*p++));
    U64Heap heap3(8 /*capacity*/, 5 /*growBy*/);
...
void U64HeapSuite::testCtor00()
{
    U64Heap heapA;
    U64Heap::item_t topItem = 0;
    bool ok = (!heapA.canGrow()) &&
        (heapA.growthFactor() == 0) &&
        (heapA.numItems() == 0) &&
        (heapA.capacity() == U64Heap::DefaultCap) &&
    ...
    U64Heap heap1(256 /*capacity*/, 0 /*growBy*/);
    for (const char* p = ITEMS; *p != 0; heap1.add(*p++));
    U64Heap heapB(0UL /*capacity*/, 0 /*growBy*/);
    heapB = heap1;
    ok = (heapB.numItems() == 0);
    CPPUNIT_ASSERT(ok);
}
...
//
void U64HeapSuite::testOp00()
{
    U64Heap heap0(256 /*capacity*/, 0 /*growBy*/);
    for (const char* p = ITEMS; *p != 0; heap0.add(*p++));

    U64Heap heap(heap0.numItems(), 0 /*growBy*/);
    heap = heap0;
    heap = heap; //no-op
...
//
void U64HeapSuite::testOp01()
{
    U64Heap heap0(256 /*capacity*/, 0 /*growBy*/);
    for (const char* p = ITEMS; *p != 0; heap0.add(*p++));

    U64Heap heap(3 /*capacity*/, -1 /*growBy*/);
    heap = heap0;
...
//
void U64HeapSuite::testOp02()
{
    U64Heap heap0(256 /*capacity*/, 0 /*growBy*/);
    for (const char* p = ITEMS; *p != 0; heap0.add(*p++));

    U64Heap heap(7 /*capacity*/, 9 /*growBy*/);
    heap = heap0;
...
//
void U64HeapSuite::testOp03()
{
    U64Heap heap0(256 /*capacity*/, 0 /*growBy*/);
    for (const char* p = ITEMS; *p != 0; heap0.add(*p++));

    U64Heap heap(7 /*capacity*/, 0 /*growBy*/);
    heap = heap0;
...
void U64HeapSuite::testResize00()
{
...
void U64HeapSuite::testRm00()
{
    ...
    {
        U64Heap::item_t item = heap.peek(i);
        U64Heap::item_t removedItem = 0;
...
void U64HeapSuite::testSort00()
{
    size_t count = sizeof(ITEMS) - 1;
    U64Heap::item_t* item = new U64Heap::item_t[count];
    size_t i = 0;
    for (const char* p = ITEMS; i < count; item[i++] = *p++);
    bool reverseOrder = false;
    U64Heap::sort(item, count, reverseOrder);

    bool ok = true;
    U64Heap::item_t prevItem = item[0];
    U64Heap::item_t curItem = 0;
...
//
void U64HeapSuite::validate(U64Heap& heap)
{
    U64Heap::item_t prevItem;
    if (heap.rm(prevItem))
    {
        U64Heap::item_t topItem;
        bool ok = true;
