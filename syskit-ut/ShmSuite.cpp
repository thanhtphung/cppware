...
void ShmSuite::testCtor00()
{
    ...
    CPPUNIT_ASSERT(ok);
    size_t size;
    ok = (m.addr(size) != 0) && (size == 4092UL);
    CPPUNIT_ASSERT(ok);
    ok = m.isCreator() && m.isOk() && (m.key() == 0) && (m.size() == 4092UL);
    CPPUNIT_ASSERT(ok);
...
void ShmSuite::testCtor01()
{
    ...
    Shm* m0 = new Shm(key, 4091UL /*size*/);
    bool ok = (m0->addr() != 0);
    CPPUNIT_ASSERT(ok);
    size_t size;
    ok = (m0->addr(size) != 0) && (size == 4091UL);
    CPPUNIT_ASSERT(ok);
    ok = m0->isCreator() && m0->isOk() && (m0->key() == key) && (m0->size() == 4091UL);
    CPPUNIT_ASSERT(ok);

    // Attach.
    Shm m1(key, 4091UL /*size*/);
    ok = (m1.addr() != 0);
    CPPUNIT_ASSERT(ok);
    size = 0;
    ok = (m1.addr(size) != 0) && (size == 4091UL);
    CPPUNIT_ASSERT(ok);
    ok = (!m1.isCreator()) && m1.isOk() && (m1.key() == key) && (m1.size() == 4091UL);
    CPPUNIT_ASSERT(ok);
...
void ShmSuite::testCtor03()
{
  ...
  {
      Shm m(key, 0UL /*size*/);
      bool ok = (m.isOk() && (!m.isCreator()) && (m.size() == 2UL));
      CPPUNIT_ASSERT(ok);
...
{
    Shm* m0 = new Shm(123UL /*size*/);
    Shm m1(*m0);
    bool ok = (m1.addr() != 0);
    CPPUNIT_ASSERT(ok);
    size_t size;
    ok = (m1.addr(size) != 0) && (size == 123UL);
    CPPUNIT_ASSERT(ok);
    ok = (!m1.isCreator()) && m1.isOk() && (m1.key() == 0) && (m1.size() == 123UL);
    CPPUNIT_ASSERT(ok);
...
void ShmSuite::testCtor05()
{
    ...
    Shm m0(key, 45UL /*size*/);
    Shm m1(m0);
    bool ok = (m1.addr() != 0);
    CPPUNIT_ASSERT(ok);
    size_t size;
    ok = (m1.addr(size) != 0) && (size == 45UL);
    CPPUNIT_ASSERT(ok);
    ok = (!m1.isCreator()) && m1.isOk() && (m1.key() == key) && (m1.size() == 45UL);
    CPPUNIT_ASSERT(ok);
...
void ShmSuite::testRm00()
{
    ...
    Shm m0(key, 124UL /*size*/);
    bool ok = m0.rm();
    CPPUNIT_ASSERT(ok);
    ok = (!m0.rm());
    CPPUNIT_ASSERT(ok);

    Shm m1(key, 124UL /*size*/);
    ok = (m1.isOk() && m1.isCreator() && (m1.size() == 124UL));
    CPPUNIT_ASSERT(ok);
}
