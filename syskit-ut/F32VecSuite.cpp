...
const char ITEM[] = "aRandomStringUsedForF32VecPopulation!!!";

BEGIN_NAMESPACE


class Sample0: public F32Vec
{
public:
    Sample0();
    ~Sample0();
...
void F32VecSuite::testAdd00()
{
    F32Vec vec0(64 /*capacity*/, 0 /*growBy*/);

    // Add random items to a fixed-capacity empty vector.
    bool ok = true;
    for (const char* p = ITEM; *p; ++p)
    {
        F32Vec::item_t item = *p;
        if (vec0.find(item))
        {
...
// Default constructor.
//
void F32VecSuite::testCtor00()
{
    F32Vec vec;
    bool ok = ((!vec.canGrow()) &&
        (vec.growthFactor() == 0) &&
        (vec.numItems() == 0) &&
        (vec.capacity() == F32Vec::DefaultCap));
    CPPUNIT_ASSERT(ok);
}


void F32VecSuite::testFind00()
{
    Sample1 vec1;

    bool ok = true;
    for (F32Vec::item_t i = 0, item = 1; item <= 32; ++i, item += 2)
    {
        if (!vec1.find(item))
        {
            ok = false;
...
void F32VecSuite::testFindKthSmallest00()
{
    ...
    const F32Vec::item_t* p = vec0.raw();
    unsigned int itemCount;
    F32Vec::item_t* item = vec0.detachRaw(itemCount);
...
//
void F32VecSuite::testOp00()
{
    Sample1 vec1;

    F32Vec vec(vec1.numItems(), 0 /*growBy*/);
    vec = vec1;
...
//
void F32VecSuite::testOp01()
{
    Sample1 vec1;

    F32Vec vec(3 /*capacity*/, -1 /*growBy*/);
    vec = vec1;
...
//
void F32VecSuite::testOp02()
{
    Sample0 vec0;
    Sample1 vec1;

    F32Vec vec(3 /*capacity*/, 3 /*growBy*/);
    vec = vec1;
...
//
void F32VecSuite::testOp03()
{
    Sample1 vec1;

    F32Vec vec(3 /*capacity*/, 0 /*growBy*/);
    vec = vec1;
    vec = vec; //no-op
    bool ok = (vec.numItems() == 3);
    CPPUNIT_ASSERT(ok);

    ok = vec1.truncate(3) && (vec1 == vec) && (vec1 == F32Vec(Sample1(), 0, 3));
    CPPUNIT_ASSERT(ok);
}


void F32VecSuite::testResize00()
{
    F32Vec vec0(64 /*capacity*/, 0 /*growBy*/);
    Sample1 vec1;
...
//
void F32VecSuite::testRm00()
{
    Sample1 vec1;
    F32Vec vec(vec1);

    // Last item.
    F32Vec::item_t item = vec1.peek(vec1.numItems() - 1);
    F32Vec::item_t removedItem = 0;
...
void F32VecSuite::testSort00()
{
    Sample1 vec1;

    F32Vec vec(vec1.numItems(), 0);
    bool reverseOrder = false;
    ...
    F32Vec::item_t prevItem = vec.peek(0);
    for (size_t i = 1, numItems = vec.numItems(); i<numItems; ++i)
    {
        F32Vec::item_t item = vec.peek(i);
