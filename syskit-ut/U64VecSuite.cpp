...
const char ITEM[] = "aRandomStringUsedForU64VecPopulation!!!";

BEGIN_NAMESPACE


class Sample0: public U64Vec
{
public:
    Sample0();
    ~Sample0();
...
void U64VecSuite::testAdd00()
{
    U64Vec vec0(64 /*capacity*/, 0 /*growBy*/);

    // Add random items to a fixed-capacity empty vector.
    bool ok = true;
    for (const char* p = ITEM; *p; ++p)
    {
        U64Vec::item_t item = *p;
        if (vec0.find(item))
        {
...
// - unsigned long U64Vec::add(size_t count, item_t item);
//
void U64VecSuite::testAdd02()
{
    unsigned long capacity = 3;
    int growBy = 0;
    U64Vec vec(capacity, growBy);
    unsigned long count = 0;
    U64Vec::item_t item = 12;
...
//
// Interfaces under test:
// - unsigned long U64Vec::add(const item_t* raw, size_t itemCount);
//
void U64VecSuite::testAdd03()
{
    unsigned long capacity = 3;
    int growBy = 0;
    U64Vec vec(capacity, growBy);
    unsigned long count = 0;
    U64Vec::item_t item[] = {12, 34, 56, 78, 99};
...
// Default constructor.
//
void U64VecSuite::testCtor00()
{
    U64Vec vec;
    bool ok = ((!vec.canGrow()) &&
        (vec.growthFactor() == 0) &&
        (vec.numItems() == 0) &&
        (vec.capacity() == U64Vec::DefaultCap));
    CPPUNIT_ASSERT(ok);
}


void U64VecSuite::testFind00()
{
    Sample1 vec1;

    bool ok = true;
    for (U64Vec::item_t i = 0, item = 1; item <= 32; ++i, item += 2)
    {
        if (!vec1.find(item))
        {
            ok = false;
...
//
// Interfaces under test:
// - U64Vec::item_t* U64Vec::detachRaw(unsigned long& numItems);
// - U64Vec::item_t U64Vec::findKthSmallest(item_t* item, size_t itemCount, size_t k);
// - const U64Vec::item_t* U64Vec::raw() const;
//
void U64VecSuite::testFindKthSmallest00()
{
    Sample0 vec0;
    unsigned long capacity = vec0.numItems();
    int growBy = 0;
    U64Vec vec1(capacity, growBy);
    bool reverseOrder = false;
    vec0.sort(vec1, reverseOrder);
    const U64Vec::item_t* p = vec0.raw();
    unsigned int itemCount;
    U64Vec::item_t* item = vec0.detachRaw(itemCount);
    bool ok = (item == p) && (itemCount == capacity);
    CPPUNIT_ASSERT(ok);

    for (unsigned long k = 0; k < capacity; ++k)
    {
        U64Vec::item_t kth = U64Vec::findKthSmallest(item, itemCount, k);
        if (kth != vec1.peek(k))
        {
            ok = false;
            break;
...
//
void U64VecSuite::testOp00()
{
    Sample1 vec1;

    U64Vec vec(vec1.numItems(), 0 /*growBy*/);
    vec = vec1;
...
//
void U64VecSuite::testOp01()
{
    Sample1 vec1;

    U64Vec vec(3 /*capacity*/, -1 /*growBy*/);
    vec = vec1;
...
//
void U64VecSuite::testOp02()
{
    Sample0 vec0;
    Sample1 vec1;

    U64Vec vec(3 /*capacity*/, 3 /*growBy*/);
    vec = vec1;
...
//
void U64VecSuite::testOp03()
{
    Sample1 vec1;

    U64Vec vec(3 /*capacity*/, 0 /*growBy*/);
    vec = vec1;
    vec = vec; //no-op
    bool ok = (vec.numItems() == 3);
    CPPUNIT_ASSERT(ok);

    ok = vec1.truncate(3) && (vec1 == vec) && (vec1 == U64Vec(Sample1(), 0, 3));
    CPPUNIT_ASSERT(ok);
}


void U64VecSuite::testResize00()
{
    U64Vec vec0(64 /*capacity*/, 0 /*growBy*/);
    Sample1 vec1;
...
//
void U64VecSuite::testRm00()
{
    Sample1 vec1;
    U64Vec vec(vec1);

    // Last item.
    U64Vec::item_t item = vec1.peek(vec1.numItems() - 1);
    U64Vec::item_t removedItem = 0;
...
void U64VecSuite::testSort00()
{
    Sample1 vec1;

    U64Vec vec(vec1.numItems(), 0);
    bool reverseOrder = false;
    ...
    U64Vec::item_t prevItem = vec.peek(0);
    for (size_t i = 1, numItems = vec.numItems(); i<numItems; ++i)
    {
        U64Vec::item_t item = vec.peek(i);
